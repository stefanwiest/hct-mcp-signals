...
class Conditions(BaseModel):
    ...
    quality_threshold: Optional[float] = Field(default=None, ge=0, le=1, description="Quality score threshold")
...
class HCTSignal(BaseModel):
    ...
    performance: Performance = Field(default_factory=Performance, description="Performance params")
    conditions: Optional[Conditions] = Field(default=None, description="Conditional params")
    ...
    def from_mcp(cls, mcp_data: Dict[str, Any]) -> "HCTSignal":
        ...
        return cls(
            ...
            performance=Performance(**sig["performance"]) if sig.get("performance") else Performance(),
            conditions=Conditions(**sig["conditions"]) if sig.get("conditions") else None,
        )
