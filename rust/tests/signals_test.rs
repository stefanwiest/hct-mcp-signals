...
use hct_mcp_signals::{
    attacca, caesura, cue, downbeat, embed_signal, extract_signal, fermata, get_json_schema,
    get_json_schema_string, get_mcp_extension_schema, tacet, vamp, HCTSignal, HoldType,
    McpTaskSend, Performance, SignalType, Tempo,
};
use serde_json::json;
...
fn test_downbeat_sync_point() {
    ...
    assert_eq!(signal.targets, vec!["*"]);
    assert_eq!(
        signal.payload.get("sync_point"),
        Some(&json!("daily_standup"))
    );
}
